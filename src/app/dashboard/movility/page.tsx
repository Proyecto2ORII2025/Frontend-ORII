...
export default function MovilityList() {
  ...
  // Variable con el blob de las mobilidades que se van a exportar
  const [fileBlob, setFileBlob] = useState<Blob | null>(null);
  // Estado del botón de exportar
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // Cargar el blob de las movilidades
        ...
        console.log("res tam:", response.length);
      } catch (error) {
              ...
              <table className="w-full text-sm">
                <thead className="bg-muted">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left font-bold text-blue">
                      Nombre
                    </th>
                    <th className="px-4 py-3 text-left font-bold text-blue">
                      Tipo de documento
                  ...
                  {movilities.map((mob) => (
                    <tr key={mob.id} className="hover:bg-muted/50">
                      ...
                      <td className="px-4 py-3">
                        {documentTypeDict[
                          mob.person.identificationType.trim().toUpperCase()
                        ] || mob.person.identificationType}
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex space-x-2">
                          ...
                          <Button onClick={() => openViewModal(mob)}>
                            Ver
                          </Button>

                          {/* Botón Editar (abre ModalEdit) */}
                          <Button onClick={() => openEditModal(mob)}>
                            Editar
                          </Button>
                        </div>
                      </td>
